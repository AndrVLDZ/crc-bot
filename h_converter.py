...
async def validate_evaluated_value(user_id: int, message: Message, value: float) -> bool:
    if isnan(value):
        menu = await main_menu(user_id)
        await message.answer(
...
async def convert_value(user_id: int, value: float, round_res: bool, message: Message) -> Union[bool, float]:
