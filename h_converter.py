    ...
    if db.check_user(user_id):
        if  await qiwi.get_rate(user_id): 
            ...
            if currency_from != currency_to: 
                ...
                parse_mode= 'Markdown', reply_markup = menu.converter_round_on)
...
async def converter(message: Message, state: FSMContext):
    ...
    if round:
        await message.answer("Enter another value", reply_markup = menu.converter_round_on)
    else:    
        await message.answer("Enter another value", reply_markup = menu.converter_round_off)