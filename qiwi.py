...
@dataclass
class Data:
    rates: List[dict] = field(default_factory=list, compare=False, hash=False, repr=False)
...
async def get_rate(user_id: int) -> str: 
    ...
    rate = [x for x in Data.rates if x['from'] == db.get_from(user_id) and x['to'] == db.get_to(user_id)]
    if (len(rate) == 0):
        return False
    else:
